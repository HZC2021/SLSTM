...
class RNN(Layer):
  ...
  def __init__(self,
    ...
    if not 'call' in dir(LSTMcell):
      raise ValueError('`cell` should have a `call` method. '
                       'The RNN was passed:', LSTMcell)
    if not 'state_size' in dir(LSTMcell):
  ...
  def states(self):
    if self._states is None:
      state = tf.nest.map_structure(lambda _: None, self.cell1.state_size)
      return state if tf.nest.is_nested(self.cell1.state_size) else [state]
    return self._states
  ...
  def compute_output_shape(self, input_shape):
    ...
    if _is_multiple_state(self.cell1.state_size):
      state_size = self.cell1.state_size
    else:
      state_size = [self.cell1.state_size]
  ...
  def build(self, input_shape):
    ...
    if isinstance(self.cell1, Layer) and not self.cell1.built:
      with backend.name_scope(self.cell1.name):
        self.cell1.build(step_input_shape)
        self.cell1.built = True
    if isinstance(self.cell2, Layer) and not self.cell2.built: ## build cell2
      with backend.name_scope(self.cell2.name):
        self.cell2.build(step_input_shape)
        self.cell2.built = True
    if isinstance(self.cell3, Layer) and not self.cell3.built:  ## build cell3
      with backend.name_scope(self.cell3.name):
        self.cell3.build(step_input_shape)
        self.cell3.built = True

    # set or validate state_spec
    if _is_multiple_state(self.cell1.state_size):
      state_size = list(self.cell1.state_size)
    else:
      state_size = [self.cell1.state_size]
  ...
  def get_initial_state(self, inputs):
    get_initial_state_fn = getattr(self.cell1, 'get_initial_state', None)
    ...
    if get_initial_state_fn:
      ...
      init_state = _generate_zero_filled_state(batch_size, self.cell1.state_size,
                                               dtype)
  ...
  def call(self,
    ...
    self._maybe_reset_cell_dropout_mask(self.cell1)
    self._maybe_reset_cell_dropout_mask(self.cell2)
    self._maybe_reset_cell_dropout_mask(self.cell3)
    ...
    kwargs = {}
    if generic_utils.has_arg(self.cell1.call, 'training'):
      kwargs['training'] = training

    # TF RNN cells expect single tensor as state instead of list wrapped tensor.
    is_tf_rnn_cell = getattr(self.cell1, '_is_tf_rnn_cell', None) is not None
    # Use the __call__ function for callable objects, eg layers, so that it
    # will have the proper name scopes for the ops, etc.
    cell_call_fn1 = self.cell1.__call__ if callable(self.cell1) else self.cell1.call
    cell_call_fn2 = self.cell2.__call__ if callable(self.cell2) else self.cell2.call
    cell_call_fn3 = self.cell3.__call__ if callable(self.cell3) else self.cell3.call
    # if constants:
    ...
    if True:
      def step(inputs, states, time):
        states = states[0] if len(states) == 1 and is_tf_rnn_cell else states
        # if time == 3 or time ==4 or time == 5:
        if time == 0 or time == 1:
          output, new_states = cell_call_fn1(inputs, states, **kwargs)
        # elif time == 4 or time == 5 or time == 6:
        elif time == 7:
          output, new_states = cell_call_fn2(inputs, states, **kwargs)
        else:
          output, new_states = cell_call_fn3(inputs, states, **kwargs)
        # tf.print("inputs:",inputs)
        # tf.print("states:", states)
        # tf.print("new_states:", new_states)
        if not tf.nest.is_nested(new_states):
          new_states = [new_states]
        return output, new_states
    last_output, outputs, states = rnn(#backend.rnn(
        step,
        inputs,
        initial_state,
        constants=constants,
  ...
  def reset_states(self, states=None):
    ...
    if tf.nest.flatten(self.states)[0] is None:
      if getattr(self.cell1, 'get_initial_state', None):
        flat_init_state_values = tf.nest.flatten(self.cell1.get_initial_state(
